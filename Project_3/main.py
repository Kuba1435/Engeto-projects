...
class WebScraper:
    ...
    def parse(self, html):
        ...
        #     election_data = requests.get(link) 
        #     content_data = soup(election_data.text, "html.parser")
        #     table2 = content_data.find_all(class_="cislo", headers="t1sa2 t1sb3")
        #     pprint(table2)
        
               
        for link in links:
            content_data = soup(requests.get(link).content, "html.parser")
            table = content_data.find_all("tbody", class_="cislo")
            pprint(table)
